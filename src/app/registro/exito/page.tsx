...
export default async function ExitoPage({ searchParams }: PageProps) {
      ...
      <div className="min-h-screen bg-black text-zinc-100 flex flex-col items-center justify-center p-4 pb-safe selection:bg-violet-600 selection:text-white">
        <div className="w-full max-w-md text-center p-8 bg-zinc-950/60 border border-zinc-900 rounded-2xl shadow-2xl space-y-6">
          ...
          <div className="pt-2">
            <Link
              href="/"
              className="flex items-center justify-center w-full min-h-[48px] rounded-xl bg-zinc-900 hover:bg-zinc-850 text-zinc-250 border border-zinc-800 text-sm font-semibold transition-all duration-150 active:scale-[0.98]"
            >
              Volver al Catálogo
            </Link>
          </div>
  ...
  return (
    <div className="min-h-screen bg-black text-zinc-100 flex flex-col items-center py-8 pb-24 px-4 selection:bg-violet-600 selection:text-white">
      ...
      <main className="w-full max-w-md flex flex-col gap-6 items-center">
        ...
        {/* Premium Ticket Card */}
        <section className="w-full relative rounded-3xl border border-zinc-800 bg-zinc-950/60 p-6 shadow-2xl flex flex-col gap-5 overflow-hidden">
          {/* Decorative Corner Dents (Punched Ticket look) */}
          <div className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-3 w-6 h-6 rounded-full bg-black border border-zinc-800 z-10" />
          <div className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-3 w-6 h-6 rounded-full bg-black border border-zinc-800 z-10" />

          {/* Ticket Header */}
          <div className="flex justify-between items-start gap-4 pb-4 border-b border-dashed border-zinc-800/80">
            <div>
          ...
          <div className="space-y-4 text-sm">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <span className="font-semibold text-zinc-350 block mt-0.5 font-mono">
              ...
              <div>
                ...
                <span className="font-semibold text-zinc-350 block mt-0.5">
                  {participant.phone}
                </span>
              </div>
            </div>
            ...
            {team && (
              <div className="p-3 bg-zinc-900/40 border border-zinc-850 rounded-xl mt-2">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                ...
                {team.leaderId === participant.id && (
                  <span className="inline-block mt-2 text-[9px] font-semibold text-violet-400 bg-violet-500/10 border border-violet-500/20 px-2 py-0.5 rounded-full">
          ...
          <div className="pt-4 border-t border-dashed border-zinc-800/80 flex flex-col items-center justify-center gap-1.5 mt-2">
            <span className="text-[10px] font-bold text-zinc-550 uppercase tracking-widest">
              Código de Confirmación
            </span>
            <span className="font-mono text-xl font-black text-white tracking-widest uppercase bg-zinc-900/80 border border-zinc-800 px-4 py-1.5 rounded-xl shadow-inner">
          ...
          <section className="w-full space-y-3">
            ...
            <div className="space-y-2 w-full">
              {event.encargados.map((enc) => (
                <div
                  key={enc.id}
                  className="flex items-center justify-between gap-3 rounded-2xl bg-zinc-950/60 p-3 border border-zinc-900"
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <span className="text-sm font-bold text-zinc-200 block line-clamp-1">
                        {enc.name}
                      </span>
                  ...
                  <a
                    ...
                    className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 text-xs font-semibold transition-all duration-150 hover:bg-emerald-500/20 active:scale-95 min-h-[44px] min-w-[44px]"
                  >
        ...
        <section className="w-full pt-4">
          <Link
            href="/"
            className="flex items-center justify-center w-full min-h-[50px] rounded-xl bg-zinc-900 hover:bg-zinc-850 text-zinc-350 hover:text-zinc-200 border border-zinc-800 text-sm font-semibold transition-all duration-150 active:scale-[0.98]"
          >
