...
export function FeriaCard({ feria }: FeriaCardProps) {
  ...
  return (
    <article
      className="group relative flex flex-col justify-end w-full aspect-[3/4] min-h-[420px] rounded-3xl border border-brand-blue/25 overflow-hidden shadow-2xl transition-all duration-300 hover:scale-[1.01] hover:border-brand-sky/40"
      style={
        imageSrc
    ...
    >
      ...
      {!imageSrc && (
        <div className="absolute inset-0 flex items-center justify-center bg-gradient-to-b from-brand-dark/70 to-brand-navy/80 text-brand-sky/25">
      ...
      <div className="absolute inset-0 bg-gradient-to-t from-black via-black/70 to-black/10 group-hover:via-black/75 transition-colors" />

      {/* Badges superiores flotantes */}
      <div className="absolute top-4 left-4 right-4 z-10 flex items-center justify-between gap-2">
        <span className="px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider bg-brand-navy/80 border border-brand-sky/25 text-brand-sky backdrop-blur-sm">
          Emprendimiento
        </span>
        <span className="px-2.5 py-0.5 rounded-full text-[10px] font-bold border bg-emerald-500/15 border-emerald-400/30 text-emerald-300 backdrop-blur-sm">
          {feria.cost}
        </span>
      </div>
      ...
      <div className="relative z-10 p-4 md:p-5 flex flex-col gap-3">
        <div className="space-y-1.5">
          <h3 className="text-lg font-black tracking-tight text-white leading-tight drop-shadow-md line-clamp-2 group-hover:text-brand-sky transition-colors">
            {feria.name}
          </h3>
          {feria.description && (
            <p className="text-[11px] leading-relaxed text-zinc-300 line-clamp-2 drop-shadow">
              {feria.description}
            </p>
          )}
        </div>
        ...
        {/* Fechas */}
        <div className="flex items-center gap-1.5 text-[11px] font-medium text-zinc-200 drop-shadow">
        ...
        {/* Coordinación (máx. 2, sobre paneles translúcidos) */}
        {feria.encargados.length > 0 && (
          <div className="flex flex-col gap-1.5">
            {feria.encargados.slice(0, 2).map((enc) => (
              <div
                key={enc.id}
                className="flex items-center justify-between gap-2 rounded-xl bg-black/40 backdrop-blur-sm border border-white/10 p-1.5 pl-2"
              >
        ...
        <div className="pt-1">
          {isExternalReg ? (
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center justify-center gap-1.5 w-full min-h-[42px] rounded-xl text-xs font-bold bg-white/10 hover:bg-white/20 text-white backdrop-blur-sm border border-white/15 transition-all active:scale-95"
            >
              Registrarse ahora
            ...
            <button
              disabled
              className="flex items-center justify-center w-full min-h-[42px] rounded-xl bg-black/40 backdrop-blur-sm border border-white/10 text-white/40 text-xs font-bold cursor-not-allowed"
            >
