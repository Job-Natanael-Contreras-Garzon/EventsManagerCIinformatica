...
export function FeriaCard({ feria }: FeriaCardProps) {
  ...
  return (
    <article className="group relative w-full rounded-3xl bg-brand-dark/40 border border-brand-blue/20 flex flex-col shadow-xl hover:border-brand-sky/30 transition-all duration-300 overflow-hidden">
      {/* Preview de imagen destacada — se muestra la foto completa */}
      <div className="relative w-full aspect-[4/3] bg-brand-navy/60 overflow-hidden">
        {imageSrc ? (
          <>
            {/* Fondo desenfocado: rellena limpio con cualquier proporción, sin franjas feas */}
            <img
              src={imageSrc}
              alt=""
              aria-hidden="true"
            ...
            />
            {/* Imagen completa, totalmente visible */}
            <img
              src={imageSrc}
              alt={feria.name}
              className="relative z-10 w-full h-full object-contain group-hover:scale-[1.03] transition-transform duration-500"
            />
          </>
        ) : (
          <div className="w-full h-full flex items-center justify-center text-brand-sky/30">
        ...
        <span className="absolute top-3 left-3 z-20 inline-block px-2.5 py-0.5 rounded-full text-[9px] font-bold border uppercase tracking-wider bg-brand-navy/70 border-brand-sky/25 text-brand-sky backdrop-blur-sm">
          Emprendimiento
        </span>
      </div>
      ...
      <div className="p-5 flex flex-col gap-4 flex-1">
        {/* Título */}
        <h3 className="text-lg font-black text-white leading-tight tracking-tight group-hover:text-brand-sky transition-colors">
          {feria.name}
        </h3>
        ...
        <div className="grid grid-cols-2 gap-3.5 border-t border-b border-brand-blue/15 py-3.5 text-xs text-brand-light-gray/80">
          ...
          <div className="flex items-center gap-2">
            <svg className="w-5 h-5 shrink-0 text-brand-sky" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
        ...
        {/* CTA fijado al fondo para alinear tarjetas en la rejilla */}
        <div className="mt-auto pt-2 border-t border-brand-blue/15">
          {isExternalReg ? (
            <a
